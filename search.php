...
<div class="container" >
							...
							<div class="col-lg-12">
								<form id="service-form" action="search.php" method="post"  style="display: block;">
									...
									<div class="form-group">
										...
										<input type="text" name="des_s"  tabindex="2" class="form-control" placeholder="descriprion">
									</div>
									<div class="form-group ">
                                        <label> du : </label>
	...
	<?php
/*     service   */
if(isset($_POST['categorie']) && isset($_POST['des_s']) && isset($_POST['d_debut']) && isset($_POST['d_fin']))
{

	$bd = new PDO('mysql:host=prodpeda-venus;dbname=hnouari;charset=utf8', 'hnouari', '18081997');
	$s = $bd->query(" select des,id_membre,id,tarif from service where categorie = '".$_POST['categorie']."' and ".
	"des like '%".$_POST['des_s']."%' and '".$_POST['d_debut']."'>= d_debut "."
	...
	while($donnee = $s->fetch()){
		...
		<input type="radio" name="service" value="<?php echo $donnee['id'];?>">  <?php echo $donnee['des']." et le mail est :".$mail['email']." tarif :".$donnee['tarif']."€/heure" ;?>  
		</div>
...
if(isset($_POST['categorie_b']) && isset($_POST['des_b']) && isset($_POST['d_debut_b']) && isset($_POST['d_fin_b']))
{
	...
	$bd = new PDO('mysql:host=prodpeda-venus;dbname=hnouari;charset=utf8', 'hnouari', '18081997');
	$s = $bd->query(" select des,id_membre,id,tarif from bien where categorie = '".$_POST['categorie_b']."' and ".
	...
	while($donnee = $s->fetch()){
		...
		$mail=$n->fetch() ;
		?> <div class = "result"  ALIGN="CENTER">
		<input type="radio" name="bien" value="<?php echo $donnee['id'] ;?>">  <?php echo $donnee['des']." et le mail est :".$mail['email']."le tarif : ".$donnee['tarif']."€/jour" ;?> 
		</div>
